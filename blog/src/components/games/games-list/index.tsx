...
import { Gamescategories } from "@/utils";
...
export default function GameList({ lists }: { lists: Games[] }) {
  ...
  const handleButtonClick = (catItem: any) => {
    ...
    router.push(`/Games/category/${catItem.value}`);
  };

  async function handleDelete(id: number) {
    console.log(id);

    const res = await fetch(`/api/game-post/delete-post?id=${id}`, {
      method: "DELETE",
      cache: "no-store",
    });
  ...
  async function helperFuncToFetchSearchResults(query: string) {
    ...
    if (data.success) {
      ...
      if (data.data.length === 0) {
        ...
        toast({
          title: "Ohh, Sorry This Game doesn't exist",
    ...
    <section className="pt-[120px]">
      <div className="container">
        <div className="flex flex-col items-center justify-center mb-12 py-10 rounded-3xl font-bold text-white w-full h-fit bg-[#7d41e1]">
          <div className="relative top-5 text-4xl">Games</div>
        ...
        <div className="-mx-4 grid grid-cols-3 gap-2">
          {searchResults && searchResults.length
            ? searchResults.map((searchGameitem: Games) => (
                <div key={searchGameitem.id} className="w-full px-4">
                  <SingleGame
                    handleDelete={handleDelete}
                    gameitem={searchGameitem}
                  />
                </div>
              ))
            : lists && lists.length
            ? lists.map((listItem: Games) => (
                <div className="px-4" key={listItem.id}>
                  <SingleGame handleDelete={handleDelete} gameitem={listItem} />
                </div>
              ))
            : null}
        </div>
