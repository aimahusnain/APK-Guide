...
export default function GameDetailsHome({ gameData }: { gameData: Games }) {
  ...
  async function handleCommentSave() {
    let extractComments = [...gameData.comments];

    extractComments.push(`${comment}|${session?.user?.name}`);

    const response = await fetch(`/api/game-post/update-post`, {
      method: "PUT",
      headers: {
                    ...
                    <div className="mr-10 mb-5 flex items-center">
                      <div className="mr-4">
                        <div className="relative h-10 w-10 overflow-hidden rounded-full">
                          <Image src={gameData?.userimage} alt="User" fill />
                        </div>
                      </div>
            ...
            <section className="dark:bg-gray-900 py-8 lg:py-16 w-full lg:w-8/12">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-lg lg:text-2xl font-bold text-black dark:text-white">
                  Comments ({gameData?.comments.length})
                </h2>
              </div>
              {gameData && gameData.comments && gameData.comments.length > 0
                      ...
                      <div className="flex justify-between items-center mb-2">
                        <div className="flex items-center">
                          <p className="inline-flex items-center mr-3 text-sm text-black dark:text-white font-semibold">
                            {comment.split("|")[1] === gameData?.userid
                              ? `${
